...
global home
user = os.path.abspath(__file__).split("/")[2]#input("Whats your current username directory on this computer?: ")
...
class Game():
	def __init__(self, dev=False, directory=""):
		...
		if int(self.preferences_data['GamingIntro-Init']):
			i = GamingIntro.Intro(dir=self.dir+"FlappyBird_Game/") #Normal Intro for game
			i.start_prg()

		if self.preferences_data['DevMode'] == "True" and dev:
			raise AttributeError("You can\'t set dev=True and preferences True in the same time")
		elif dev:
			self.dev = True
		else:
			self.dev = True if self.preferences_data['DevMode'] == 'True' else False
		...
		def hsc():
			...
			h = HighscoreManager.Highscore(users=os.listdir("/home"), pathinit=home)
			hs = h.getTable()
		...
		def w_fe():
			f = FeedbackInterface.Feedback(pathinit=home, users=os.listdir("/home"))
			f.start()
		def s_fe():
			f = FeedbackInterface.Feedback(pathinit=home, users=os.listdir("/home"))
			f.see_feedbacks()
	...
	def load_data_pref(self):
		...
		self.tk.bind(self.preferences_data["Kick-Key"], ball.kick)
		self.current_pref = [self.preferences_data['Kick-Key']] + self.preferences_data['Bindings'].split(";")
	...
	def keys(self, evt):
		...
		if evt.char == self.current_pref[1] or evt.keysym == self.current_pref[1]: #for pause game
			...
			if self.pause:
				self.gameIsRunning = False
				print("Game paused")
				messagebox.showinfo("OMGames", "Game paused, press the same key to resume")
...
#=========
	...
	messagebox.showinfo("Game", "Game will start when you click the ok button or %s key" % g.preferences_data["Kick-Key"])
...
#==================|Introduction|======================<
	...
	for text in texts:
		...
		labels.append(l)
	def com():
		messagebox.showinfo("OMGames", "Have fun! And beat your highscores!", master=intk)
		intk.destroy()
		return
